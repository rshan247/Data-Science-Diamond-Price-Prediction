...
def train_using_gradient_boosting(x_train, x_test, y_train):
    gradient_boosting_model = GradientBoostingRegressor(n_estimators=300, random_state=42, learning_rate=0.1, loss="absolute_error")
...
def y_prediction_visualization(x_test, y_test, y_prediction):
    pred_df = pd.DataFrame({'Actual': y_test, 'Predicted': y_prediction})
    sns.pairplot(pred_df, kind='kde')
...
def main():
    ...
    gradient_boosting_prediction = train_using_gradient_boosting(x_train, x_test, y_train)

    # Model Prediction
    y_prediction = predict(x_test, gradient_boosting_prediction)
