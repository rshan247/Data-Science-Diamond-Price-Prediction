...
def y_prediction_visualization(x_test, y_test, y_prediction):
    sns.scatterplot(x=x_test['carat'], y=y_test, label="Actual values", color='green')
    sns.lineplot(x=x_test['carat'], y=y_prediction, color='blue', label="Predicted values")
    plt.xlabel("Carat")
    plt.ylabel("Price")
...
def main():
    ...
    KNN_prediction = train_using_KNN(x_train, x_test, y_train)

    # Model Prediction
    y_prediction = predict(x_test, KNN_prediction)
